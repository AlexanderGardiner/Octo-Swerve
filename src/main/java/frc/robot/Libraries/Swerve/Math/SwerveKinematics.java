...
public class SwerveKinematics {
     ...
     * 
     * @param modulePositions The positions of the modules relative to the center of
     *                        the robot
     */
     ...
     * @return The calculated swerve module states (For the robot gyro zeroed facing
     *         away from the drivers)
     */
    public SwerveModuleState[] calculateFromChassisSpeeds(ChassisSpeeds chassisSpeeds) {
        SwerveModuleState[] moduleStates = { new SwerveModuleState(), new SwerveModuleState(), new SwerveModuleState(),
        ...
        for (int i = 0; i < modulePositions.length; i++) {
            ...
            Translation2d targetModulePosition = new Translation2d(
                    modulePositions[i].getX() * Math.cos(targetLocation.getRotation().getRadians())
            ...
            // Translating the swerve module positions to the translation of the target
            // location
            ...
            // Calculating the distance for the modules to travel, and the angle they need
            // to travel at
            double distanceToTargetModulePosition = Math
                    .sqrt(Math.pow((targetModulePosition.getY() - modulePositions[i].getY()), 2)
                            + Math.pow((targetModulePosition.getX() - modulePositions[i].getX()), 2));
            double angle = (Math.atan2(targetModulePosition.getX() - modulePositions[i].getX(),
                    targetModulePosition.getY() - modulePositions[i].getY()));
