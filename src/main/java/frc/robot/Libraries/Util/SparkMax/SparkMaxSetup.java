...
public class SparkMaxSetup {
    ...
    public static void setup(CANSparkMax sparkMax, SparkMaxConfig sparkMaxConfig) {
        ...
        for (int i=0; i<sparkMaxConfig.getStatusFrames().getFrames().size(); i++) {
            sparkMax.setPeriodicFramePeriod(PeriodicFrame.values()[i], sparkMaxConfig.getStatusFrames().getFrames().get(i));
        }
        ...
        if (sparkMaxConfig.getEncoderType() == SparkMaxEncoderType.Absolute) {
            ...
            sparkMax.getPIDController().setFeedbackDevice(sparkMax.getAlternateEncoder(sparkMaxConfig.getEncoderCountsPerRev())); 
            sparkMax.getAlternateEncoder(sparkMaxConfig.getEncoderCountsPerRev()).setInverted(sparkMaxConfig.getSensorInverted());
        } else {
